...
the operator's Mermaid is the first companion character integrated into the Personal World UI. She serves as the digital resident — a comforting, always-present mascot that lives in the sidebar navigation and appears contextually in empty/loading states.

More companions are coming (Little Helper robot, VEFR Norse squirrel, Tacos & the morning paper truck). The mermaid is the operator's special companion for the operator's theme; other companions will map to other themes.
...
```
Theme Pack Layer
├── the operator Theme → Mermaid companion
